...
class AddPostAll extends StatefulWidget {
  AddPostAll({super.key});

  @override
  State<AddPostAll> createState() => _AddPostAllState();
}

class _AddPostAllState extends State<AddPostAll> {
  ...
  Widget build(BuildContext context) {
    final addpostcontroller = Get.put(Addpostcontrollerall());
    final homeController = Get.put(HomeController());
    final themeController = Get.put(ThemeController());
    final areaController = Get.put(AreaController());
    final userData = Get.put(Userdahsboardcontroller());

    return GetX<Addpostcontrollerall>(
      builder: (controller) => Visibility(
        visible: controller.showAddAllPost.value,
        child: Scaffold(
                ...
                children: [
                  ...
                  Expanded(
                    child: PageView(
                      ...
                      children: [
                        _buildStep1(controller, userData, themeController),
                        _buildStep2(controller, homeController, areaController,
                            themeController),
  ...
  AppBar _buildAppBar(
      ThemeController themeController, Addpostcontrollerall add) {
    return AppBar(
      leading: IconButton(
  ...
  Widget _buildStepProgress(Addpostcontrollerall controller) {
    return Obx(
      () => Container(
  ...
  Widget _buildStep1(
    Addpostcontrollerall controller,
        ...
        child: Form(
          ...
          child: SingleChildScrollView(
            padding: EdgeInsets.all(16.w).copyWith(
              bottom:
                  MediaQuery.of(context).viewInsets.bottom + 620.h, // ديناميكي
            ),
            child: Column(
              children: [
  ...
  Widget _buildPublisherDropdown(
      Addpostcontrollerall controller, Userdahsboardcontroller userData) {
  ...
  Widget _buildCategoryDropdowns(Addpostcontrollerall controller) {
    return Column(
      children: [
        Obx(() => Visibility(
  ...
  Widget _buildStep3(
      Addpostcontrollerall controller, ThemeController themeController) {
    return SingleChildScrollView(
      physics: const BouncingScrollPhysics(),
      keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag,
      padding: EdgeInsets.all(16.w).copyWith(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: 20.h),
          Text(
            "هل يحتوي المنشور على مبلغ مالي؟".tr,
            style: TextStyle(
              fontSize: 16.sp,
              fontFamily: AppTextStyles.DinarOne,
              color: AppColors.redColor,
          ...
          Obx(() => Row(
                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                children: [
                  _buildPriceToggleButton(
                    text: "نعم".tr,
                    isSelected: controller.isAddPrice.value,
                    onPressed: () => controller.isAddPrice.value = true,
                  ),
                  _buildPriceToggleButton(
                    text: "لا".tr,
                    isSelected: !controller.isAddPrice.value,
                    onPressed: () => controller.isAddPrice.value = false,
                  ),
                ],
              )),
          SizedBox(height: 20.h),
          ...
          PostDetailsField(
            ...
            controller: controller.isAddPrice.value
                ? controller.detailControllersWithPrice["تفاصيل اضافية"]
                : controller.detailControllers["تفاصيل اضافية"],
            keyboardType: TextInputType.text,
          ),
        ],
      ),
      ...
      child: Column(
        children: [
          ...
          if (controller.isAddPrice.value)
            _buildReviewItem(
                'السعر'.tr,
                controller.detailControllersWithPrice["السعر"]!.text
                    .toString()),
  ...
  // أزرار التنقل
  Widget _buildNavigationButtons(Addpostcontrollerall controller,
          ...
          children: [
            ...
            if (controller.currentStep.value < 4)
              _buildNavigationButton(
                ...
                isPrimary: true,
                onPressed: () {
                  _validateAndProceed(controller);
                  setState(() {});
                },
              ),
            if (controller.currentStep.value == 4)
  ...
  // زر الإرسال النهائي
  Widget _buildSubmitButton(Addpostcontrollerall controller,
  ...
  void _validateAndProceed(Addpostcontrollerall controller) {
    controller.currentStep.value++;
