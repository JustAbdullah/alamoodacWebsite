...
class SubscriptionController extends GetxController {
  ...
  try {
    ...
    final url = Uri.parse('https://alamoodac.com/modac/public/subscriptions/request');
...
Future<void> generateSubscriptionCode(int subscriptionId) async {
  try {
    ...
    // 3. إرسال الطلب
    final response = await http.post(
      Uri.parse('https://alamoodac.com/modac/public/subscriptions/generate-code'),
      headers: {'Content-Type': 'application/json'},
...
final RxList<SubscriptionCode> codes = <SubscriptionCode>[].obs;
  ...
  Future<void> fetchCodes(int subscriptionId) async {
    try {
      isLoading.value = true;
      final response = await http.get(
        Uri.parse('https://alamoodac.com/modac/public/subscriptions/codes/$subscriptionId'),
        headers: {'Accept': 'application/json'},
      );
...
Future<void> redeemSubscriptionCode(BuildContext context) async {
  ...
  try {
    final response = await http.post(
      Uri.parse('https://alamoodac.com/modac/public/subscriptions/redeem-code'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'user_id': userId,
