...
class AddpostCarController extends GetxController {
...
Future<void> uploadImagesToServer() async {
  ...
  final Map<String, TextEditingController> detailControllers = {
    ...
    "النوع": TextEditingController(),
    "سنة الصنع": TextEditingController(),
    "الكليومترات": TextEditingController(),
    "حالة السيارة": TextEditingController(),
    "الموديل": TextEditingController(),
    "نوع الوقود": TextEditingController(),
    "نوع ناقل الحركة": TextEditingController(),
    "سعة المحرك": TextEditingController(),
    "عدد المقاعد": TextEditingController(),
    "اللون الخارجي": TextEditingController(),
    "اللون الداخلي": TextEditingController(),
    "نوع الهيكل": TextEditingController(),
    "تفاصيل اضافية": TextEditingController(),
    "حالة الهيكل": TextEditingController(),
    "إمكانية الإيجار": TextEditingController(),
  };
...
Future<void> _translateDetailWithRetries(
  ...
  Future<void> createPost(Map<String, dynamic> postData) async {
    try {
      ...
      // التحقق من حالة الاستجابة 200 أو 201
      if (response.statusCode == 200 || response.statusCode == 201) {
        var responseBody = json.decode(response.body);
        var postId = responseBody['id']; // Assuming 'id' is the returned ID
        print("Post created successfully with ID: $postId");

        loading.value = false;
        Get.snackbar("نجحت العملية".tr, "تم إنشاء المنشور بنجاح".tr,
            snackPosition: SnackPosition.BOTTOM, backgroundColor: Colors.green);
      } else {
        print("Failed to create post. Status Code: ${response.statusCode}");
        print("Response Body: ${response.body}");
        loading.value = false;
        Get.snackbar("فشلت العملية".tr,
      ...
      Get.snackbar("خطا في العملية".tr, "خطا في العملية: $e",
          snackPosition: SnackPosition.BOTTOM, backgroundColor: Colors.red);
    }
  }

  //////////////////////////////// القائمة المنسدلة للاقسام الفرعية الأولى والثانية
  ///
  ...
  Future<void> executePostCreation(var storeId, var long, var lat,
    ...
    // التحقق من الحقول قبل بدء العملية
    if (categoryId == 0) {
      Get.snackbar("خطأ".tr, "يرجى اختيار القسم الرئيسي".tr,
    ...
    if (images.isEmpty) {
      Get.snackbar("خطأ".tr, "يرجى رفع الصور قبل إنشاء المنشور".tr,
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: AppColors.redColor);
      Navigator.of(context, rootNavigator: true).pop();
    ...
    if (titleController.text.trim().isEmpty) {
      Get.snackbar("خطأ".tr, "يرجى إدخال العنوان".tr,
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: AppColors.redColor);
      Navigator.of(context, rootNavigator: true).pop();
    ...
    // التحقق من تفاصيل المنشور
    for (var entry in detailControllers.entries) {
      if (entry.value.text.trim().isEmpty) {
        Get.snackbar("خطأ".tr, "يرجى إدخال ${entry.key}",
            snackPosition: SnackPosition.BOTTOM,
            backgroundColor: AppColors.redColor);
        Navigator.of(context, rootNavigator: true).pop();
    ...
    if (uploadedImageUrls.isEmpty) {
      loading.value = false;
      Get.snackbar("خطا".tr, "خطا في الصورة حاول رفع صور أخرى بصيغة مختلفة".tr,
    ...
    if (translatedDetails.isEmpty) {
      loading.value = false;
      Get.snackbar(
          "خطا في المحتويات".tr, "خطا في ترجمة النصوص حاول مجددًا لاحقًا".tr,
          backgroundColor: Colors.red);
      Navigator.of(context, rootNavigator: true).pop();
  ...
  List<String> carBrandsArabic = [
    // ماركات عالمية مشهورة
    'تويوتا',
    'فولكسفاغن',
    'فورد',
    'هوندا',
    ...
    // ماركات الشاحنات والمركبات التجارية
    'إيسوزو',
    'سكانيا',

    // ماركات تاريخية / توقفت عن الإنتاج
    'ديلوريان',
    'أولدزموبيل',

    // خيار "غير مذكور" أو "أخرى"
    'غير مذكور',
    'أخرى',
  ];
