...
class Addpostcontrollerauction extends GetxController {
  RxBool showAdd = false.obs;final 
...
Future<void> _translateDetailWithRetries(
  ...
  String convertDateTimeToString(DateTime? dateTime) {
    // استخدام التاريخ الحالي في حالة كانت القيمة null
...
//////////
  ...
  Future<void> createPost(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // الحصول على ID المنشور من داخل كائن post
        var postId = responseBody['post']['id'];

        print("Post created successfully with ID: $postId");

        auctionData['post_id'] = postId.toString();
        // استدعاء وظيفة لإنشاء المزاد
       ...
       Get.find<LoadingController>(). useFreePost( Get.find<LoadingController>().currentUser?.id??0):null;
        ...
        Get.snackbar(
          "خطأ".tr,
          "فشل إنشاء المنشور. الكود: ${response.statusCode}\n${response.body}",
  ...
  Future<void> createAuction(Map<String, dynamic> auctionData) async {
    try {
      print("Starting auction creation...");
      var response = await http.post(
        Uri.parse("https://alamoodac.com/modac/public/auctions"),
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Get.snackbar(
          "خطأ".tr,
          "فشل إنشاء المزاد: ${response.body}",
          snackPosition: SnackPosition.BOTTOM,
...
String? expiresAt =  Get.find<LoadingController>().currentUser?.free_post_used == 0
    ...
    var auctionData = {
      'start_price': convertArabicToEnglishNumbers(
          detailControllers["السعر"]!.text.toString()),
      'start_time':
          convertDateTimeToString(startDateTime), // تحويل الوقت إلى String
      'end_time':
    ...
    // إنشاء المنشور
    await createPost(postData, auctionData);
    Navigator.of(context, rootNavigator: true).pop();
