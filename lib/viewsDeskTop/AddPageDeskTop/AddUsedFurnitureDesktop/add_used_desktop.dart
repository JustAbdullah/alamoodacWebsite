...
class AddUsedDesktop extends StatelessWidget {
  AddUsedDesktop({super.key});
  ...
  AppBar _buildAppBar(
      ThemeController themeController, AddpostUsedController add) {
    return AppBar(
      leading: IconButton(
  ...
  Widget _buildStepProgress(AddpostUsedController controller) {
    return Obx(
      () => Container(
        width: 600.w,
  ...
  Widget _buildStep1(
    ...
    AddpostUsedController controller,
  ...
  Widget _buildPublisherDropdown(
      AddpostUsedController controller, Userdahsboardcontroller userData) {
  ...
  Widget _buildCategoryDropdowns(AddpostUsedController controller) {
    return SizedBox(
          width: 600.w,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.center,
  ...
  Widget _buildStep3(BuildContext context, AddpostUsedController controller,
      ThemeController themeController) {
    return SingleChildScrollView(
      physics: const BouncingScrollPhysics(),
      keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag,
      ...
      child: SizedBox(
        ...
        children: [
          ...
          PostDetailsField(
            maxLines: 2,
            label: "المكان".tr,
            hint: "أدخل المكان او موقعك بشكل صحيح مثال: بغداد-شرق دجلة".tr,
            controller: controller.detailControllers["المكان"],
            keyboardType: TextInputType.text,
          ),
          ...
          DropdownField(
            ...
            onChanged: (value) {
              controller.detailControllers["الحالة"]?.text = value!;
              print('تم اختيار: $value');
            },
          ),
          SizedBox(height: 10.h),
          DropdownField(
            label: "طريقة البيع".tr,
            items: [
              "غير مدخل".tr,
              "كاش".tr,
              "تقسيط".tr,
            ...
            selectedItem: "غير مدخل".tr,
            onChanged: (value) {
              controller.detailControllers["طريقة البيع"]?.text = value!;
              print('تم اختيار: $value');
            },
          ),
  ...
  Widget _buildStep4(BuildContext context, AddpostUsedController controller,
      ThemeController themeController) {
    return SingleChildScrollView(
      physics: const BouncingScrollPhysics(),
      keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag,
  ...
  Widget _buildImageItem(AddpostUsedController controller, int index) {
    return Stack(
      children: [
       Image.memory(
  ...
  // زر الإرسال النهائي
  Widget _buildSubmitButton(AddpostUsedController controller,
  ...
  // إرسال الكائن كامل
                ...
                Get.find<AddpostUsedController>().hideAll();
                Get.find<AddpostUsedController>().resetAll(); // تم التعديل هنا
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value),
  ...
  void _validateAndProceed(AddpostUsedController controller) {
    controller.currentStep.value++;
