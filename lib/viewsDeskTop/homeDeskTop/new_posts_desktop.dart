...
class NewPostsDesktop extends StatefulWidget {
  const NewPostsDesktop({super.key});

  @override
  _NewPostsDesktopState createState() => _NewPostsDesktopState();
}

class _NewPostsDesktopState extends State<NewPostsDesktop> {
    ...
    return Row(
      children: [
        Icon(
          ...
          size: 30.sp, // تصغير حجم الأيقونة قليلاً
        ),
        SizedBox(width: 12.w), // تقليل المسافة هنا
        Text(
          "أحدث المنشورات".tr,
          style: TextStyle(
            fontFamily: AppTextStyles.DinarOne,
            color: AppColors.textColor(themeController.isDarkMode.value),
            fontSize: 24.sp, // تصغير حجم النص قليلاً
  ...
  Widget _buildContentSection() {
    return Obx(() {
      if (controller.LoadingPosts.value) return _buildWebShimmerLoader();
      if (controller.postsList.isEmpty) return _buildWebEmptyState();
