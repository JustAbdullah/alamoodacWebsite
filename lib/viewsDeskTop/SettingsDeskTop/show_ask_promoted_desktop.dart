...
class ShowAskPromotedDeskTopPage extends StatelessWidget {
  const ShowAskPromotedDeskTopPage({super.key});

  @override
  Widget build(BuildContext context) {
    final Userdahsboardcontroller userDahsboardController = Get.put(Userdahsboardcontroller());
    ...
    return Obx(
      () => AnimatedSwitcher(
        ...
        child:
            _buildMainContainer(userDahsboardController,
                themeController, context, settingsController, selectedPostId, isSubmitting)
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  ...
                  SizedBox(height: 40.h),
                  _buildActionButtonSection(userDahsboardController,
                      themeController, selectedPostId, isSubmitting, controller),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [  _buildCloseIcon(themeController, controller),
               SizedBox(
              width: 15.w,
            ), SizedBox(
              width: 250.w,
...
Widget _buildCloseIcon(
  ...
  Widget _buildPromotionHeroSection(ThemeController themeController) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.05),
        borderRadius: BorderRadius.circular(25),
        border: Border.all(
          color: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.15),
          width: 1.5,
        ),
      ),
      child: Column(
      ...
      children: [
        Container(
          ...
          child: Column(
            children: [
              Expanded(
                child: _buildPostPreview("العرض العادي", false, themeController),
              ),
              Container(
                height: 4.h,
                color: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.1),
              ),
              Expanded(
                child: _buildPostPreview("المنشور الممول", true, themeController),
              ),
            ],
          ),
        ),
  ...
  Widget _buildPostPreview(String title, bool isPromoted, ThemeController themeController) {
    return Container(
      padding: EdgeInsets.all(15.w),
      color: isPromoted ? AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.08) : Colors.transparent,
      child: Row(
        children: [
          Container(
            width: 50.w,
  ...
  return GridView.count(
    ...
    children: [
        _buildFeatureItem(Icons.visibility, "وضوح أعلى".tr, "زيادة معدلات المشاهدة".tr),
        _buildFeatureItem(Icons.groups, "جمهور أوسع".tr, "وصول لمتابعين جدد".tr),
       _buildFeatureItem(
  Icons.attach_money,
  "تمويل مرن".tr,
  "سهوله في الطلب وبشكل فوري".tr
),
        _buildFeatureItem(Icons.schedule, "مدة الترويج".tr, "عالي ويصل لاسابيع او اشهر".tr),
      ],
    );
  }

  Widget _buildFeatureItem(IconData icon, String title, String subtitle) {
  return IntrinsicHeight( // يجعل العناصر متساوية الارتفاع
    child: Container(
  ...
  Widget _buildPostSelectionSection(ThemeController themeController, RxInt selectedPostId) {
    return Obx(
      () => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget _buildPostItems() {
    ...
    return  DropdownFieldApi(
              ...
              items: [
                "غير مدخل".tr,
                ...userData.postsList.map(
                    (store) => store.translations.first.title).toList(),
              ],
              selectedItem:
                  userData.postsList.isNotEmpty ? "غير مدخل".tr : null,
              onChanged: (value) {
    ...
    return Column(
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: isActive ? AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.2) : Colors.transparent,
            shape: BoxShape.circle,
            border: Border.all(
              color: isActive ? AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value) : Colors.grey.withOpacity(0.3),
              width: 2,
            ),
          ),
          child: Icon(icon, 
  ...
  Widget _buildTimelineConnector() {
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(
          color: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.2),
   ...
   
  {
    return Obx(
      () => ElevatedButton(
        onPressed: isSubmitting.value ? null : () async {  PromotedadController promotedadController  = Get.put(PromotedadController());
        
          if (userData. selectedIdPost.value == null||userData. selectedIdPost.value ==0) {
            Get.snackbar(
              "خطأ".tr,
              "يرجى اختيار منشور أولاً".tr,
              snackPosition: SnackPosition.BOTTOM,
              backgroundColor: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.9),
       ...
       await    promotedadController.createAd( userData.selectedIdPost?.value??0);
        ...
        style: ElevatedButton.styleFrom(
          ...
          elevation: 5,
          shadowColor: AppColors.backgroundColorIconBack(Get.find<ThemeController>().isDarkMode.value).withOpacity(0.3),
          textStyle: TextStyle(
            fontFamily: AppTextStyles.DinarOne,
            fontSize: 20.sp,
            fontWeight: FontWeight.bold,
